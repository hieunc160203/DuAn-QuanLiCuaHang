 ...
 */
public class CapNhatNhanVienJFrame extends javax.swing.JFrame {

    private static NguoiDung model;

    /**
     * Creates new form CapNhatNhanVienJFrame
     */
    public CapNhatNhanVienJFrame(NguoiDung model) {
        initComponents();
        this.model = model;
        inIt();
    }
    ...
    private void initComponents() {
        ...
        txt_TenDangNhap.setBackground(new java.awt.Color(255, 255, 204));
        ...
        lbl_NhapLaiMatKhau.setText("Nhập lại mật khẩu mới");

        txt_NhapLaiMatKhau.setFont(new java.awt.Font("Tahoma", 0, 18)); // NOI18N
        txt_NhapLaiMatKhau.setEnabled(false);
        ...
        chk_CapNhatMatKhau.addItemListener(new java.awt.event.ItemListener() {
            public void itemStateChanged(java.awt.event.ItemEvent evt) {
                chk_CapNhatMatKhauItemStateChanged(evt);
            }
        });
    ...
    private void chk_CapNhatMatKhauItemStateChanged(java.awt.event.ItemEvent evt) {//GEN-FIRST:event_chk_CapNhatMatKhauItemStateChanged
        // TODO add your handling code here:
        if (chk_CapNhatMatKhau.isSelected()) {
            txt_MatKhau.setEnabled(true);
            txt_NhapLaiMatKhau.setEnabled(true);
        } else {
            txt_MatKhau.setEnabled(false);
            txt_NhapLaiMatKhau.setEnabled(false);
        }
    }//GEN-LAST:event_chk_CapNhatMatKhauItemStateChanged
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(CapNhatNhanVienJFrame.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                new CapNhatNhanVienJFrame(model).setVisible(true);
            }
        });
    }
    ...
    /*
     * Hàm đổ thông tin người dùng lên textfield
     */
    void doThongTin() {
        txt_TenDangNhap.setText(model.getTenDangNhap());
        ...
        dat_NgaySinh.setDate(model.getNgaySinh());
        cmb_VaiTro.setSelectedIndex(model.getVaiTro());
    ...
    void xoaThongTin() {
        ...
        txt_MatKhau.setBackground(Color.white);
        txt_NhapLaiMatKhau.setBackground(Color.white);
        chk_CapNhatMatKhau.setSelected(false);
        txt_MatKhau.setText("");
    ...
    boolean checkDienThoai() {
        ...
        try {
            ...
            } else if (!NguoiDungDAO.kiemSDT(SDT) && !SDT.equals(model.getDienThoai())) {
                ketqua = false;
    ...
    boolean checkEmail() {
        try {
            ...
            if (!NguoiDungDAO.kiemEmail(email) && !email.equals(model.getEmail())) {
    ...
    /*
     * Hàm kiểm lỗi trước khi cập nhật Nhanvien
     */
    boolean kiemLoi() {
    ...
    NguoiDung taoModel() {
        ...
        ND.setTenDangNhap(txt_TenDangNhap.getText().trim());
        ND.setHoTen(txt_HoTen.getText());
        ND.setDienThoai(txt_DienThoai.getText());
        ND.setGioiTinh(rad_Nam.isSelected());
        ND.setNgaySinh(dat_NgaySinh.getDate());
        ND.setEmail(txt_Email.getText().trim());
        ND.setDiaChi(txa_DiaChi.getText());
        ND.setVaiTro(cmb_VaiTro.getSelectedIndex());
        ND.setTrangThai(cmb_TrangThai.getSelectedIndex() == 0);
        return ND;
    }

    /*
     * Hàm cập nhật thông tin người dùng lên database
          sau khi cập nhật thì close frame này
     */
    void capnhatNguoiDung() {
        try {
            ...
            if (kiemLoi()) {
                int rs = 0;
                // tạo model:
                NguoiDung ND = taoModel();
                // kiem tra co cap nhat mat khau moi hay khong
                // neu co thi cap nhat vao model 
                if (chk_CapNhatMatKhau.isSelected()) {
                    String MatKhauMoi = new String(txt_MatKhau.getPassword());
                    if (checkMatKhau()) {
                        ND.setMatKhau(MatKhauMoi);
                        rs = NguoiDungDAO.capnhatNguoiDung(ND, true);
                    }
                } else {
                    rs = NguoiDungDAO.capnhatNguoiDung(ND, false);
                }
                if (rs != 0) {
